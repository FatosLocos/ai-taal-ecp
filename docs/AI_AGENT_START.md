...
Expected baseline: 79 passing tests and split sizes `14336/1024/1024`.
...
Do not modify ECP-0 through ECP-6 or the completed ECP-7 Batch 1 through Batch 11
...
the strongest base. The confirmatory ECP-7 test is still sealed.

Continue with this sequence:

1. Read `docs/research-design-ecp7.md` and `docs/development-log-ecp7.md`.
2. Define exactly one ECP7-B12 intervention and its failure criterion.
...
Batch 10 is the strongest weak-structure result. Batch 11 showed that reducing
late utilization pressure harms task accuracy, code use and sender agreement.
B10's one-hidden-layer population decoder and fresh translator both retain a
size weakness, while color is the other residual population error. The most
informative next step is one generic decoder-capacity change. A clean ECP7-B12
progression is:

- keep the same world and bit budget;
- return to the complete B10 training and loss schedule;
- preserve the flattened position-aware input and shared representation;
- add exactly one second shared hidden layer to both receiver and translator;
...
Do not add a factor-specific input, binding matrix, slot selection or loss
change. Do not change the sender, hidden width, data, temperature, duration,
validation cadence or development thresholds, and do not combine decoder depth
