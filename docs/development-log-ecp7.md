...
Early-stopping patience: `5,000` steps<br>
Hard-meaning predicate: all `16` current population links have an incorrect factor<br>
Replay receiver-parameter gradients: **disabled**<br>
Ordinary-task receiver-parameter gradients: **enabled**<br>
Replay weight: `0` through step `15,000`, linear to `0.25` at step `20,000`, then hold<br>
Replay batch size: `64` meanings, uniform with replacement<br>
Training-only pool refresh interval: `200` steps<br>
Replay sampler seed offset: `+1211`<br>
Raw configuration SHA-256: `7d91bb825564681f059ba207766d0a392cc26bafaf2b113d5354c249abd1d2a0`<br>
Effective configuration SHA-256: `518ff24f4e1d625ff09366b9515dd5359a3a34fc7e8e801cca5dde40dd8f5acc`<br>
Split-SHA-256: `4947058c75ab07cb43a87eb82776b12cb2a7e2eeba7114de110d3b852cbc64cd`<br>
Test unsealed: **no**

ECP7-B22-I inherits Batch 21 and changes only
`receiver_parameter_gradients` from its implicit default `true` to `false` for
the additional replay loss. A gradient-routing invariant test confirms that the
replay branch produces non-zero sender gradients, no receiver-parameter
gradients, and restores receiver trainability for the ordinary task. The
positive control is rerun, and no alternative routing or accompanying
intervention is admitted into this batch.

## Batch 22 results

Positive-control run: `runs/ecp7-batch22-control-development/20260718T162953Z-ecp7-development`<br>
Intervention run: `runs/ecp7-batch22-intervention-development/20260718T162953Z-ecp7-development`<br>
Test unsealed: **no**

| Metric | Positive control | ECP7-B22-I |
|---|---:|---:|
| Population train, mean | 100% | **83.6796%** |
| Population train, worst link | 100% | 82.6800% |
| Population validation, mean | 100% | **83.5083%** |
| Population validation, worst link | 100% | 82.3242% |
| Universal translator, validation | 100% | **83.5938%** |
| Exact sender-message agreement | 100% | 92.06% |
| Unique messages per sender | 15,360 | 12,705–13,200 |
| Collision meanings per sender | 0 | 2,160–2,655 |
| Message entropy | 13.91 bits | 13.54–13.61 bits |
| Development gate | pass | **fail** |

The positive control passed every gate at 100%. ECP7-B22-I passes validation and
translator thresholds but fails both train thresholds and sender injectivity.
It does not improve any primary performance metric over Batch 21. The joint gate
fails and confirmatory access remains unauthorized.

All 76 optimization records through step 15,000 match Batch 21 exactly across
the 31 shared fields. Every B22 history record also confirms
`receiver_parameter_gradients=false` for the replay branch. The initial all-link
pool therefore matches B21 at 1,745 meanings before routing can act.

Sender-only routing changes the pool in the hypothesized direction. The pool
falls to 1,579 by full weight at step 20,000, reaches a minimum 1,410 at step
20,800, and contains 1,513 meanings at the selected step 24,600. The selected
checkpoint has task loss `0.05441`, replay loss `0.21320`, weight `0.25`, and
utilization loss `-0.75974`; the replay contribution is approximately `0.0533`,
almost identical to task loss. Patience stops training at step 29,600, with
1,598 all-link failures and 83.30% validation.

The pool reduction does not improve total reconstruction. A selected-checkpoint
diagnostic finds 1,513 meanings failed by all links, down from Batch 21's 1,997,
but 3,197 meanings failed by at least one link, up from 2,755. Mean failed links
per hard meaning drops from 13.57 to 11.71. Thus sender-only replay moves codes
away from population-shared errors but redistributes the failures into
link-specific disagreement faster than ordinary receiver training can follow.

Compared with Batch 21, mean train falls by 0.03 percentage points, worst-link
train by 0.11 points, mean validation by 0.12 points, worst-link validation by
0.29 points, translator validation by 0.05 points, and sender agreement by 0.90
points. Batch 21 retains the strongest validation and agreement results.

Selected sender codebooks contain 12,838, 12,855, 12,705 and 13,200 unique
messages, with 2,522, 2,505, 2,655 and 2,160 collision meanings. Population
validation factor accuracies were `[99.82%, 100%, 98.44%, 85.22%]`;
universal-translator factor accuracies were
`[100%, 100%, 98.44%, 85.16%]`. The persistent size and texture bottlenecks are
unchanged.
...
ECP7-B22-I is a valid negative result with a useful mechanistic distinction.
Blocking receiver replay gradients reduces the target shared-error pool, so
joint coadaptation was part of B21's pool growth. However, sender-only routing
merely converts shared failures into cross-link fragmentation and does not raise
train exactness or injectivity. The confirmatory split remains sealed.

A future ECP7-B23 may keep every Batch 22 setting while making gradient routing
two-phase: sender-only from steps 15,000–20,000, then restore receiver-parameter
gradients for replay after the registered warmup. This tests whether an initial
...
Early-stopping patience: `5,000` steps<br>
Hard-meaning predicate: all `16` current population links have an incorrect factor<br>
Replay sender-message gradients: **enabled through step 20,000; disabled from step 20,001**<br>
Replay receiver-parameter gradients: **disabled through step 20,000; enabled from step 20,001**<br>
...
Effective configuration SHA-256: `a01f184249b32859074909039ba86d495854a3c5f063c19f5519e2739135cb9e`<br>
Split-SHA-256: `4947058c75ab07cb43a87eb82776b12cb2a7e2eeba7114de110d3b852cbc64cd`<br>
Test unsealed: **no**

ECP7-B24-I inherits Batch 23 and changes only sender-message gradient routing
for the additional replay branch after step 20,000. Replay is sender-only
through the registered warmup and receiver-only afterward. Receiver-only replay
detaches the sampled sender messages before reconstruction, while the ordinary
task continues to update both senders and receivers. Gradient invariant tests
cover each isolated replay phase and both ordinary-task routes. The positive
control is rerun, and no alternative switch point, routing, predicate or
accompanying intervention is admitted into this batch.

## Batch 24 results

Positive-control run: `runs/ecp7-batch24-control-development/20260718T171239Z-ecp7-development`<br>
Intervention run: `runs/ecp7-batch24-intervention-development/20260718T171239Z-ecp7-development`<br>
Test unsealed: **no**

| Metric | Positive control | ECP7-B24-I |
|---|---:|---:|
| Population train, mean | 100% | **84.6854%** |
| Population train, worst link | 100% | 82.5614% |
| Population validation, mean | 100% | **84.4849%** |
| Population validation, worst link | 100% | 82.2266% |
| Universal translator, validation | 100% | **84.5459%** |
| Exact sender-message agreement | 100% | 91.50% |
...
The positive control passed every gate at 100%. ECP7-B24-I sets new ECP-7
development bests for mean train, mean validation and universal-translator
validation. It still fails both train thresholds and sender injectivity, while
worst-link validation is below Batch 21. The joint gate fails and confirmatory
access remains unauthorized.

All 101 optimization records through step 20,000 match Batch 23 and Batch 22
exactly across their shared fields. Every history record confirms the
registered routes: sender-only replay through step 20,000 and receiver-only
replay from the next boundary at step 20,200. The all-link pool follows the
common trajectory from 1,745 to a
minimum of 1,414 at step 18,200 and 1,579 at step 20,000.

The first receiver-only boundary still observes 1,984 all-link failures, because
pool mining precedes that boundary's update. Unlike joint B23, subsequent
receiver-only updates reduce that pool again: it contains 1,990 meanings at
step 22,000, 1,690 at the selected step 29,400 and 1,710 at the final step
30,000. The selected checkpoint is at the end of the available horizon rather
than an early-stopping plateau.

The selected diagnostic finds 1,690 meanings failed by all links and 3,142
failed by at least one link. Mean failed links among those meanings is 11.18.
This is close to Batch 22's sender-only pattern of 1,513 shared and 3,197
any-link failures, but the receiver catch-up phase raises mean train by 1.01
percentage points and mean validation by 0.98 points relative to B22. It does
not repair the fragmented worst-link bottleneck.

Compared with Batch 21, B24 improves mean train by 0.98 percentage points, mean
validation by 0.85 points and translator validation by 0.90 points. Worst-link
train falls by 0.23 points, worst-link validation by 0.39 points, and sender
agreement by 1.46 points. Receiver-only replay therefore improves the average
code-reader fit while increasing population imbalance.

Selected sender codebooks contain 13,214, 12,955, 12,720 and 13,440 unique
messages, with 2,146, 2,405, 2,640 and 1,920 collision meanings. Population
validation factor accuracies were `[99.98%, 99.87%, 98.44%, 86.17%]`;
universal-translator factor accuracies were
`[100%, 100%, 98.44%, 86.11%]`. Texture improves, while the fixed size
collision pattern and sender-specific texture differences remain.
...
ECP7-B24-I is a valid negative result and the strongest mean-performance ECP-7
development variant so far. Receiver-only catch-up is better than joint replay
for mean reconstruction, but it does not solve injectivity or worst-link
generalization. The confirmatory split remains sealed.

The selected checkpoint occurs at step 29,400 and training reaches the 30,000
step cap before the registered 5,000-step patience can evaluate a late plateau.
A future ECP7-B25 may inherit Batch 24 unchanged and extend only `max_steps` to
45,000. This isolates whether the receiver-only phase is still improving or
merely approaching a non-injective plateau. No coefficient, route, predicate,
architecture, optimizer or data setting may change with that horizon test.

## Batch 25 preregistration
...
Effective configuration SHA-256: `00c9f13c5919f98f5b81c14c8ea2d852a1cd4be55ecbfcaf0d61abed6b9acc91`<br>
Split-SHA-256: `4947058c75ab07cb43a87eb82776b12cb2a7e2eeba7114de110d3b852cbc64cd`<br>
Test unsealed: **no**

ECP7-B25-I inherits Batch 24 and changes only `training.max_steps` from
`30,000` to `45,000`. The registered 5,000-step early-stopping patience remains
active, so 45,000 is a ceiling rather than a required duration. All replay
routes, timing, weights, data and selection criteria remain unchanged. The
positive control is rerun, and no alternative horizon or accompanying
intervention is admitted into this batch.

## Batch 25 results

Positive-control run: `runs/ecp7-batch25-control-development/20260718T172746Z-ecp7-development`<br>
Intervention run: `runs/ecp7-batch25-intervention-development/20260718T172746Z-ecp7-development`<br>
Test unsealed: **no**

| Metric | Positive control | ECP7-B25-I |
|---|---:|---:|
| Population train, mean | 100% | **85.2631%** |
| Population train, worst link | 100% | 82.8125% |
| Population validation, mean | 100% | **85.2417%** |
| Population validation, worst link | 100% | 82.7148% |
| Universal translator, validation | 100% | **85.1318%** |
| Exact sender-message agreement | 100% | 92.16% |
| Unique messages per sender | 15,360 | 12,720–13,440 |
| Collision meanings per sender | 0 | 1,920–2,640 |
| Message entropy | 13.91 bits | 13.54–13.64 bits |
| Development gate | pass | **fail** |

The positive control passed every gate at 100%. ECP7-B25-I improves every
primary B24 accuracy and sender agreement, setting new ECP-7 development bests
for mean and worst-link train, mean and worst-link validation, and translator
validation. It still fails both train thresholds and sender injectivity. The
joint gate fails and confirmatory access remains unauthorized.

All 151 optimization records through step 30,000 match Batch 24 exactly. The
extended receiver-only phase raises mean validation from 84.45% at step 30,000
to the selected 85.24% at step 43,800. Training reaches the 45,000-step ceiling
with 85.09% validation, so the registered patience still does not terminate the
run before the cap. Improvement after step 30,000 is real but slow and noisy.

The all-link pool contains 1,710 meanings at step 30,000, 1,691 at step 40,000,
1,677 at selection and 1,659 at the final step. The selected diagnostic finds
1,677 meanings failed by all links and 2,967 failed by at least one link, with
11.39 mean failed links per erroneous meaning. Relative to B24, the longer
phase removes 175 any-link failures but only 13 shared failures.

The selected codebooks explain the plateau. Their unique-message counts are
13,440, 12,960, 12,720 and 13,440. Even a perfect receiver can therefore decode
at most 87.50%, 84.38%, 82.81% and 87.50% of each sender's 15,360 training
meanings. The mean code-cap is 85.55%; observed mean train is already 85.26%,
and the observed worst link exactly equals the lowest 82.81% code-cap. Longer
receiver-only optimization cannot satisfy the train gates without first
changing sender collisions.

Compared with Batch 24, B25 improves mean train by 0.58 percentage points,
worst-link train by 0.25 points, mean validation by 0.76 points, worst-link
validation by 0.49 points, translator validation by 0.59 points and sender
agreement by 0.66 points. Receiver induction has caught up to the existing
codes, but the remaining performance gap is structurally sender-side.

Selected sender codebooks contain 13,440, 12,960, 12,720 and 13,440 unique
messages, with 1,920, 2,400, 2,640 and 1,920 collision meanings. Population
validation factor accuracies were `[99.99%, 100%, 98.44%, 86.82%]`;
universal-translator factor accuracies were
`[100%, 100%, 98.44%, 86.69%]`. The stable size collisions and sender-specific
texture caps remain the limiting structure.
...
ECP7-B25-I is a valid negative result and the strongest ECP-7 development
variant so far. The extended catch-up phase improves receiver fit but reaches
the mathematical ceiling imposed by non-injective sender codebooks. The
confirmatory split remains sealed, and another horizon-only extension is not
justified.

A future ECP7-B26 may test one deterministic coordinate-descent cycle while
keeping Batch 25's 45,000-step ceiling: sender-only replay through step 20,000,
receiver-only through 30,000, a second sender-only separation pulse through
35,000, and receiver-only catch-up afterward. The second sender pulse reuses the
